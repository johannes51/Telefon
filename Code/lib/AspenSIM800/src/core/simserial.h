...
public:
    ...
    void end();                                 // Closes SoftwareSerial.
    void write(const char*);                    // Writes provided char[] to SIM800 module. Does not append "\r\n" and does not call read().
    void print(const char*);                    // Prints provided char[] to SIM800 module. Appends "\r\n" and calls read().
