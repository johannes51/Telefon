...
public:
    bool reply(const char*);              // Returns true if char[] arg is found in ioBuffer.
...
private:
    ...
    void outBuilder(CmdType, const char *, const char *, bool = true);    // Called by each AT wrapper method. Builds the AT cmd which is sent to the SIM800 module.
